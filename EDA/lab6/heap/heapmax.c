...
int *cria_listaordenada (Heap *heap) {
  int* v = (int*)malloc(sizeof(int) * heap->pos);
  if(v == NULL){
    printf("Erro ao alocar v (cria_listaordenada!");
    exit(1);
  }

  for (int i = heap->pos-1; i > -1; --i)
  {
    v[i] = heap->prioridade[0];

    if(heap_remove(heap) == -1){
      i = -1;
    }
  }
...
int* heap_sort (int nums[], int tam) {
  ...
  int *novosnums = cria_listaordenada(heap);
  heap_libera(heap);
  return novosnums;
}  
