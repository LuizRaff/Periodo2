...
int* cmcBellmanFord (Grafo *grafo, int no_inicial, int* cmccusto) {
  ...
  for (int k = 1; k < grafo->nv; k++) {
    ...
    for (int i = 0; i < grafo->nv; i++) {
      viz_aux = grafo->viz[i];
      while (viz_aux) {
        relaxa(cmccusto, cmcvant, i, viz_aux->noj, viz_aux->peso);
        viz_aux = viz_aux->prox;
      }
    }
  }
  return cmcvant;
...
int* cmcDijkstraBuscaLinear (Grafo *grafo, int no_inicial, int* cmccusto) {
  ...
  **/
  ...
  while(corrente != -1 && cmccusto[corrente] != INT_MAX) {
    ...
    **/
    // Q3: COMPLETAR
    viz_aux = grafo->viz[corrente];
    while (viz_aux) {
      relaxa(cmccusto, cmcvant, corrente, viz_aux->noj, viz_aux->peso);
      viz_aux = viz_aux->prox;
    }
  ...
  **/
  ...
  while(corrente != -1 && cmccusto[corrente] != INT_MAX) {
    ...
    **/
    ...
    viz_aux = grafo->viz[corrente];
    while (viz_aux) {
      if (relaxa(cmccusto, cmcvant, corrente, viz_aux->noj, viz_aux->peso)) {
        heap_corrige(heap, cmccusto[viz_aux->noj], viz_aux->noj);
      }
      viz_aux = viz_aux->prox;
    }
